...
public static class Shapes {
    ...
    // data container to hold vectorized positions and normals
	...
	public struct Plane : IShape {
        //takes an index and resolutions and calculates vectorized positions and normals
        // for the shape and returns point4
    ...
    //<S> turns the job into a generic job allowing it to be a template for multiple jobs
    // it is named S because we are making shapes
    // where S:struct, IShape limits S to only work with structs of interface IShape
    // default(S) is the generic type whereas the explicit type was normally used
